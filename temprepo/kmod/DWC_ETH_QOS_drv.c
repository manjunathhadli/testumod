...
void DWC_ETH_QOS_drv_init_pci_base_addr(ULONG base_addr)
{
	pci_base_addr = base_addr;
}
...
void DWC_ETH_QOS_stop_all_ch_tx_dma(struct DWC_ETH_QOS_prv_data *pdata)
{
	...
	for(i = 0; i < DWC_ETH_QOS_TX_QUEUE_CNT; i++) {
		retval = hw_if->stop_dma_tx(i);
		if (retval != Y_SUCCESS)
			printk(KERN_ALERT "Failed to stop DMA channel %d\n", i);
	}
...
static void DWC_ETH_QOS_tx_desc_mang_ds_dump(struct DWC_ETH_QOS_prv_data *pdata)
{
	...
	for (qInx = 0; qInx < DWC_ETH_QOS_TX_QUEUE_CNT; qInx++) {
		...
		for (i = 0; i < TX_DESC_CNT; i++) {
			...
			printk(KERN_ALERT "\t[%4p %d] = %#x : %#x : %#x : %#x\n",
					...
					UINT qInx)
{
	...
	/* initializes MAC and DMA */
	hw_if->init(pdata);	//TODO: init only one channel which generated
...
irqreturn_t DWC_ETH_QOS_ISR_SW_DWC_ETH_QOS(int irq, void *device_id)
{
	...
	/* handle user space queue/channel interrupt */
	for (qInx = pdata->rx_avb_q_idx; qInx < pdata->max_rx_queue_cnt; qInx++) {
		DMA_SR_RgRd(qInx, varDMA_SR);
		DMA_IER_RgRd(qInx, varDMA_IER);
		...
		if (GET_VALUE(varDMA_SR, DMA_SR_RI_LPOS, DMA_SR_RI_HPOS) & 1) {
			...
			printk(KERN_ALERT "Got receive interrupt on queue %d(%d)\n",
					sem_idx, qInx);
				    ...
				    gfp_t gfp)
{
	...
	skb = dev_alloc_skb(pdata->rx_buffer_len);
	if (skb == NULL) {
		printk(KERN_ALERT "Failed to allocate skb\n");
		return -ENOMEM;
	}
	...
	unsigned int qInx)
{
	...
	for (retry = 0; retry < 20; retry++) {
		...
		if (ret == 1) {
			...
			DBGPR_PTP("Device has not yet updated the context "
					"desc to hold the Rx time time stamp(retry = %d)\n", retry);
		} else {
			printk(KERN_ALERT "Rx time stamp is corrupted(retry = %d)\n", retry);
			return 2;
		}
	}
	...
	if (retry == 20) {
			printk(KERN_ALERT "Device has not yet updated the context "
				"desc to hold Rx time stamp(retry = %d, qInx = %d)\n",
                                retry, qInx);
			desc_data->dirty_rx--;
		...
		if ((hw_if->get_tx_desc_ls(txptr)) && !(hw_if->get_tx_desc_ctxt(txptr))) {
			...
			if (hw_if->tx_carrier_lost_error) {
				if (hw_if->tx_carrier_lost_error(txptr)) {
					DBGPR_PTP("tx carrier lost error\n");
					err_incremented = 1;
...
#ifdef YDEBUG_FILTER
static void DWC_ETH_QOS_check_rx_filter_status(struct s_RX_NORMAL_DESC *RX_NORMAL_DESC, UINT qInx)
{
	...
	if ((rdes3 & 0x8000000) == 0x8000000) {
		...
		if ((rdes2 & 0x8000) == 0x8000)
			printk(KERN_ALERT "VLAN pkt passed VLAN filter(qInx = %d)\n", qInx);
...
int DWC_ETH_QOS_poll(struct DWC_ETH_QOS_prv_data *pdata, int budget, int qInx)
{
	...
	struct net_device *dev = pdata->dev;
	int received = 0;
	USHORT lro_in_effect = 0;
	struct hw_if_struct *hw_if = &pdata->hw_if;
	struct desc_if_struct *desc_if = &pdata->desc_if;

	DBGPR("-->DWC_ETH_QOS_poll: budget = %d, qInx = %u\n",
		budget, qInx);

	while (received < budget) {
		struct sk_buff *skb = NULL;
		struct DWC_ETH_QOS_rx_buffer *buffer =
		    GET_RX_BUF_PTR(qInx, desc_data->cur_rx);
		...
		if (varrx_error_counters == 0) {
			...
			/* write metadata & then pass to receive level */
			skb->dev = dev;
			skb->protocol = eth_type_trans(skb, dev);
			skb_record_rx_queue(skb, qInx);

			if ((pdata->rx_pkt_features.
			     pkt_attributes & DWC_ETH_QOS_RX_CHECKSUM_DONE) ==
			    DWC_ETH_QOS_RX_CHECKSUM_DONE)
			...
			if (dev->features & NETIF_F_HW_VLAN_RX) {
				if ((pdata->rx_pkt_features.pkt_attributes & DWC_ETH_QOS_RX_VLAN_PKT) ==
						DWC_ETH_QOS_RX_VLAN_PKT) {
					USHORT vlan_tag = 0;
					/* get VLAN tag */
					RX_PKT_FEATURES_VLAN_TAG_VT_Mlf_Rd(pdata->rx_pkt_features.vlan_tag, vlan_tag);
...
#ifdef YDEBUG_FILTER
			DWC_ETH_QOS_check_rx_filter_status(RX_NORMAL_DESC, qInx);
#endif
			...
			if (skb->dev->features & NETIF_F_LRO) {
				lro_in_effect = 1;
				lro_receive_skb(&desc_data->lro_mgr, skb,
						(void *)pdata);
			} else {
				napi_gro_receive(&pdata->rx_queue[qInx].napi, skb);
			}
		} else {
			DBGPR("Error in received pkt, hence failed to pass it to upper layer\n");
			dev_kfree_skb(skb);
			dev->stats.rx_errors++;
		...
		desc_data->dirty_rx++;
		if (desc_data->dirty_rx >= MIN_RX_DESC_CNT)
	...
	if (lro_in_effect)
		lro_flush_all(&desc_data->lro_mgr);

	DBGPR("<--poll: received = %d\n", received);
...
*
* \details This function is called in poll function to update the status of received packets.
*
* \param[in] dev - pointer to net_device structure.
				 ...
				 unsigned int rx_status)
{
	...
	/* rx length error */
	if ((rx_status & DWC_ETH_QOS_RX_LENGTH_ERR) ==
	    DWC_ETH_QOS_RX_LENGTH_ERR)
		dev->stats.rx_length_errors++;

	/* receiver ring buffer overflow */
	if ((rx_status & DWC_ETH_QOS_RX_BUF_OVERFLOW_ERR) ==
	    DWC_ETH_QOS_RX_BUF_OVERFLOW_ERR)
		dev->stats.rx_over_errors++;

	/* received pkt with crc error */
	if ((rx_status & DWC_ETH_QOS_RX_CRC_ERR) == DWC_ETH_QOS_RX_CRC_ERR)
		dev->stats.rx_crc_errors++;

	/* received frame alignment */
	if ((rx_status & DWC_ETH_QOS_RX_FRAME_ERR) == DWC_ETH_QOS_RX_FRAME_ERR)
		dev->stats.rx_frame_errors++;

	/* receiver fifo overrun */
	if ((rx_status & DWC_ETH_QOS_RX_FIFO_OVERFLOW_ERR) ==
	    DWC_ETH_QOS_RX_FIFO_OVERFLOW_ERR)
		dev->stats.rx_fifo_errors++;

	/* receiver missed packet */
	if ((rx_status & DWC_ETH_QOS_RX_MISSED_PKT_ERR) ==
	    DWC_ETH_QOS_RX_MISSED_PKT_ERR)
			...
			UINT qInx)
{
	...
	while (received < quota) {
		...
		if (!(RX_NORMAL_DESC->RDES3 & DWC_ETH_QOS_RDESC3_OWN)) {
			...
			/* Rceive Checksum Offload configuration */
			skb->ip_summed = CHECKSUM_NONE;
		...
		if (!(RX_NORMAL_DESC->RDES3 & DWC_ETH_QOS_RDESC3_OWN)) {
			...
			if (!(RX_NORMAL_DESC->RDES3 & DWC_ETH_QOS_RDESC3_ES) &&
				...
				if ((pdata->dev_state & NETIF_F_RXCSUM) == NETIF_F_RXCSUM) {
					/* Receive Status RDES1 Valid ? */
					if ((RX_NORMAL_DESC->RDES3 & DWC_ETH_QOS_RDESC3_RS1V)) {
						/* check(RDES1.IPCE bit) whether device has done
						 * csum correctly or not */
						RX_NORMAL_DESC_RDES1_Ml_Rd(RX_NORMAL_DESC->RDES1, varRDES1);
						if ((varRDES1 & 0xC8) == 0x0)
...
static int DWC_ETH_QOS_set_features(struct net_device *dev, netdev_features_t features)
{
	...
	/* No need to disable vlan tag insertion on TX path explicitly as it is taken
	 * care by vlan_tx_tag_present() API. This API returns 1 for all vlan packets
	 * if txvlan is enabled and returns 0 if txvlan is disabled.
	 * */
	...
	if (((features & NETIF_F_HW_VLAN_TX) == NETIF_F_HW_VLAN_TX)
		...
		/* for TSO feature */
	if ((changed & NETIF_F_SG) == NETIF_F_SG) {
		dev->hw_features |= NETIF_F_SG;
		printk(KERN_ALERT "State change - sg enable\n");
	} else {
		dev->hw_features &= ~NETIF_F_SG;
		printk(KERN_ALERT "State change - sg disable\n");
	}
		/* for SG feature */
	if ((changed & NETIF_F_TSO) == NETIF_F_TSO) {
		if (pdata->hw_feat.tso_en == 0)
		  return -EOPNOTSUPP;
		dev->hw_features |= NETIF_F_TSO;
		printk(KERN_ALERT "State change - tso enable\n");
	} else {
		dev->hw_features &= ~NETIF_F_TSO;
		printk(KERN_ALERT "State change - tso disable\n");
	}
		...
		struct ifr_data_struct *req)
{
	...
	DBGPR_FILTER("Successfully %s VLAN %s filtering\n",
...
static INT DWC_ETH_QOS_change_mtu(struct net_device *dev, INT new_mtu)
{
	...
	if (max_frame <= 2048)
		...
		pdata->rx_buffer_len = 4096;	/* in case of JUMBO frame,
		  ...
		  int last_desc_idx, int flag, UINT qInx)
{
	...
	if (first_desc_idx == last_desc_idx) {
		...
		for (i = first_desc_idx; i <= last_desc_idx;) {
...
static void DWC_ETH_QOS_mmc_setup(struct DWC_ETH_QOS_prv_data *pdata)
{
	 ...
	 * */
	hw_if->disable_mmc_interrupts();

	if (pdata->hw_feat.mmc_sel) {
		hw_if->config_mmc_counters();
...
/* SW timer function, which call *_tx_interrupt() fun to clean tx desc */
void DWC_ETH_QOS_tx_timer(unsigned long data)
{
	struct DWC_ETH_QOS_prv_data *pdata = (struct DWC_ETH_QOS_prv_data *)data;
	struct net_device *dev = pdata->dev;
	UINT qInx;

	DBGPR("-->DWC_ETH_QOS_tx_timer\n");

	for (qInx = 0; qInx < DWC_ETH_QOS_TX_QUEUE_CNT; qInx++) {
		DWC_ETH_QOS_tx_interrupt(dev, pdata, qInx);
	}

	mod_timer(&pdata->tx_wd_timer, DWC_ETH_QOS_COAL_TIMER(pdata->tx_coal_timer));

	DBGPR("<--DWC_ETH_QOS_tx_timer\n");
}

/* set TX coalesce parameters and sw timers */
void DWC_ETH_QOS_init_tx_coalesce(struct DWC_ETH_QOS_prv_data *pdata)
{
	DBGPR("-->DWC_ETH_QOS_init_tx_coalesce\n");

	pdata->tx_count_frames = 0;
	pdata->tx_coal_frames = DWC_ETH_QOS_TX_MAX_FRAMES;
	pdata->tx_coal_timer = DWC_ETH_QOS_COAL_TX_TIMER;
	init_timer(&pdata->tx_wd_timer);
	pdata->tx_wd_timer.expires = DWC_ETH_QOS_COAL_TIMER(pdata->tx_coal_timer);
	pdata->tx_wd_timer.data = (ULONG)pdata;
	pdata->tx_wd_timer.function = DWC_ETH_QOS_tx_timer;
	add_timer(&pdata->tx_wd_timer);
