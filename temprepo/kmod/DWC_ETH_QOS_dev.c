...
void DWC_ETH_QOS_dev_init_pci_base_addr(ULONG base_addr)
{
...
static int get_lpi_status(void)
{
  unsigned long varmac_lps;
                                 ...
                                 INT perfect_hash)
{
  ...
  MAC_VLANTR_VTHM_UdfWr(perfect_hash);
  /* To enable only HASH filtering then VL/VID
   * should be > zero. Hence we are writting 1 into VI.
   * It means MAC will always receive VLAN pkt with
...
* Returns 1 if timestamp is valid else returns 0
* \param[in] rxdesc
* \return Success or Failure
* \retval  0 Success
* \retval -1 Failure
...
static INT enable_dma_interrupts(UINT qInx)
{
	...
	varDMA_IER = varDMA_IER | ((0x1) << 0) | ((0x1) << 1) | ((0x1) << 2) |
	    ((0x1) << 6) | ((0x1) << 7) | ((0x1) << 8) | ((0x1) << 15) |
	    ((0x1) << 16) | ((0x1) << 12);

	DMA_IER_RgWr(qInx, varDMA_IER);

#if defined(DWC_ETH_QOS_TXPOLLING_MODE_ENABLE)
	/* TIE - Transmit Interrupt Disable */
	DMA_IER_TIE_UdfWr(qInx, 0x0);
#endif
...
u64 DWC_ETH_QOS_get_hwclock(void)
{
  ...
  MAC_STSR_TSS_UdfRd(timehigh);
  MAC_STNSR_TSSS_UdfRd(timelow);

  ns = ((unsigned long long)timehigh * 1000000000ull) + (unsigned long long)timelow;
				...
				UINT qInx)
{
	 ...
	 * segment could be reset and the timer re-started to invok the
	 * *_tx_interrupt() fun.
	 */
	pdata->tx_count_frames += GET_CURRENT_XFER_DESC_CNT(qInx);
	if (pdata->tx_coal_frames > pdata->tx_count_frames) {
		/* reset IC */
		TX_NORMAL_DESC_TDES2_IC_Mlf_Wr(TX_NORMAL_DESC->TDES2, 0x0);
		DBGPR("[%d]: tx_count_frames %d\n", last_index, pdata->tx_count_frames);
		//mod_timer(&pdata->tx_wd_timer,
		//           DWC_ETH_QOS_COAL_TIMER(pdata->tx_coal_timer));
	} else {
...
static INT configure_mtl_queue(UINT qInx, struct DWC_ETH_QOS_prv_data *pdata)
{
	...
	p_rx_fifo = calculate_per_queue_fifo(pdata->hw_feat.rx_fifo_size,
			DWC_ETH_QOS_MAX_RX_Q_CNT);
	p_tx_fifo = calculate_per_queue_fifo(pdata->hw_feat.tx_fifo_size,
			DWC_ETH_QOS_MAX_TX_Q_CNT);
