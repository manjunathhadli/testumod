...
static int DWC_ETH_QOS_alloc_queue_struct(struct DWC_ETH_QOS_prv_data *pdata)
{
	...
	DBGPR("-->DWC_ETH_QOS_alloc_queue_struct: max_tx_queue_cnt = %d,"\
		"max_rx_queue_cnt = %d\n", pdata->max_tx_queue_cnt,
		pdata->max_rx_queue_cnt);

	pdata->tx_queue =
		kzalloc(sizeof(struct DWC_ETH_QOS_tx_queue) * DWC_ETH_QOS_MAX_TX_Q_CNT,
		GFP_KERNEL);
	...
	pdata->rx_queue =
		kzalloc(sizeof(struct DWC_ETH_QOS_rx_queue) * DWC_ETH_QOS_MAX_RX_Q_CNT,
		GFP_KERNEL);
			...
			UINT qInx)
{
	...
	desc_data->skb_realloc_threshold = 0;
	...
	/* LRO configuration */
	desc_data->lro_mgr.dev = pdata->dev;
	memset(&desc_data->lro_mgr.stats, 0, sizeof(desc_data->lro_mgr.stats));
	desc_data->lro_mgr.features = LRO_F_NAPI;
	desc_data->lro_mgr.ip_summed = CHECKSUM_UNNECESSARY;
	desc_data->lro_mgr.ip_summed_aggr = CHECKSUM_UNNECESSARY;
	desc_data->lro_mgr.max_desc = MAX_LRO_DESC;
	desc_data->lro_mgr.max_aggr = MAX_LRO_AGGR;
	desc_data->lro_mgr.lro_arr = desc_data->lro_arr;
	desc_data->lro_mgr.get_skb_header = DWC_ETH_QOS_get_skb_hdr;
	memset(&desc_data->lro_arr, 0, sizeof(desc_data->lro_arr));
...
* \param[in] **iphdr - pointer to be pointed to the ip header,
...
* \return integer
*
* \retval -1 if the packet does not conform to ip protocol, else 0
*/
					...
					struct sk_buff *skb)
{
	...
	while (len) {
		...
		if (size > DWC_ETH_QOS_MAX_DATA_PER_TX_BUF) {
			...
			/* fill the second buffer pointer */
			buffer = GET_TX_BUF_PTR(qInx, index);
			...
			/* fill the first buffer pointer only */
			buffer->dma = dma_map_single((&pdata->pdev->dev),
								 (skb->data + offset),
								 size,
								 DMA_TO_DEVICE);
	...
	for (i = 0; i < frag_cnt; i++) {
		...
		struct page *p = frag->page.p;

		buffer = GET_TX_BUF_PTR(qInx, index);
		/* fill the first buffer pointer */
		buffer->dma =
		    dma_map_page((&pdata->pdev->dev), p,
		...
		if ((i+1) < frag_cnt) {
			...
			if (dma_mapping_error((&pdata->pdev->dev),
					buffer->dma2)) {
				printk(KERN_ALERT "failed to do the dma2 map\n");
				goto err_out_dma_map_fail;
			}
			buffer->len2 = frag->size;
		} else {
			buffer->dma2 = 0;
			buffer->len2 = 0;
		}
				     ...
				     struct DWC_ETH_QOS_tx_buffer *buffer)
{
	...
	if (buffer->dma) {
		if (buffer->mapped_as_page == Y_TRUE) {
