...
/* Uncomment below macro to enabled Software Tx watchdog timer */
#define DWC_ETH_QOS_TIWDT
...
/* The following macros map Vayavya's error macros to POSIX errno values */
...
#define DWC_ETH_QOS_MAX_HDR_SIZE DWC_ETH_QOS_HDR_SIZE_64B
...
//#define DWC_ETH_QOS_MAX_DATA_PER_TX_BUF (1 << 14)     /* 16 KB Maximum data per buffer pointer(in Bytes) */
...
//#define DWC_ETH_QOS_MAX_DATA_PER_RX_BUF (1 << 14)     /* 16 KB Maximum data per buffer pointer(in Bytes) */
#define DWC_ETH_QOS_MAX_DATA_PER_RX_BUF (1 << 12)	/* for tetsing purpose: 4 KB Maximum data per buffer pointer(in Bytes) */
#define DWC_ETH_QOS_MAX_DATA_PER_RXD (DWC_ETH_QOS_MAX_DATA_PER_RX_BUF * 2)	/* Maxmimum data per descriptor(in Bytes) */

#define DWC_ETH_QOS_MAX_SUPPORTED_MTU 9000
...
#define DWC_ETH_QOS_RDESC2_HL	0x000007FF
...
#define DWC_ETH_QOS_MAX_TX_Q_CNT (pdata->max_tx_queue_cnt)
#define DWC_ETH_QOS_RX_QUEUE_CNT (pdata->rx_queue_cnt)
#define DWC_ETH_QOS_MAX_RX_Q_CNT (pdata->max_rx_queue_cnt)
#define DWC_ETH_QOS_QUEUE_CNT min(DWC_ETH_QOS_TX_QUEUE_CNT, DWC_ETH_QOS_RX_QUEUE_CNT)
#define DWC_ETH_QOS_MAX_QUEUE_CNT min(DWC_ETH_QOS_MAX_TX_Q_CNT, DWC_ETH_QOS_MAX_RX_Q_CNT)
...
#define DWC_ETH_QOS_COAL_TX_TIMER    2 /* 2 msec */
/* Tx Max coalesced frames,
 * If it is zero it works in normal mode where each desc will
 * have IC bit set
 * */
#define DWC_ETH_QOS_TX_MAX_FRAMES 16
#define DWC_ETH_QOS_COAL_TIMER(x) (jiffies + msecs_to_jiffies(x))
...
/* for EEE */
#define DWC_ETH_QOS_DEFAULT_LPI_LS_TIMER 0x3E8
...
struct hw_if_struct {
	...
	INT(*config_vlan_filtering)(INT filter_enb_dis, INT perfect_hash);
...
struct DWC_ETH_QOS_rx_wrapper_descriptor {
	...
	struct net_lro_desc lro_arr[MAX_LRO_DESC];
...
struct DWC_ETH_QOS_prv_data {
	...
	UCHAR tx_queue_cnt;
	UCHAR max_tx_queue_cnt;
	UINT tx_qInx;

	/* RX Queue */
	struct DWC_ETH_QOS_rx_queue *rx_queue;
	UCHAR rx_queue_cnt;
	UCHAR max_rx_queue_cnt;
	UINT rx_qInx;
...
#define DWC_ETH_QOS_FLOW_CTRL_TX_RX (DWC_ETH_QOS_FLOW_CTRL_TX |\
	...
	/* INCR and INCRX mode */
	/* set to 1 for INCRX mode and 0 for INCR mode */
	UINT incr_incrx;

	/* for hanlding jumbo pkts on rx path */
  ...
  bool one_nsec_accuracy; /* set to 1 if onw nano second accuracy
	...
	u32 tx_avb_queue;
	u32 rx_avb_queue;
	unsigned int tx_avb_q_idx;
	unsigned int rx_avb_q_idx;
	int bar_no;
	struct semaphore rx_sem[DWC_ETH_QOS_MAX_AVB_Q_CNT];
#endif /* end of DWC_ETH_QOS_CONFIG_UMODEDRV */

#ifdef DWC_ETH_QOS_TIWDT
	/* for tx coalesce schem */
	struct timer_list tx_wd_timer; /* SW timer for tx interrupt mitigation */
	u32 tx_count_frames; /* latest no of TX desc which have IC bit reset */
	u32 tx_coal_frames; /* Max TX desc cnt with IC reset */
	u32 tx_coal_timer; /* SW timer expire value */
...
void DWC_ETH_QOS_dev_init_pci_base_addr(ULONG);
void DWC_ETH_QOS_drv_init_pci_base_addr(ULONG);
void DWC_ETH_QOS_debug_init_pci_base_addr(ULONG);
...
u64 DWC_ETH_QOS_get_hwclock(void);
#ifdef DWC_ETH_QOS_TIWDT
...
void DWC_ETH_QOS_pg_init_pci_base_addr(ULONG base_addr);
...
#ifdef YDEBUG_CHDRV
#define DBGPR_CHDRV(x...) printk(KERN_ALERT x)
#else
#define DBGPR_CHDRV(x...) do {} while (0)
#endif
